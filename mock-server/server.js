...
router.get("/vrcs/:vrc/components", (req, res) => {
  ...
  console.log("Fetching components:", { type, package: pkg, module });
  if (!type || !pkg || !module) {
    return res.status(400).json({ error: "Missing type, package, or module" });
  }
...
router.post("/vrcs/:vrc/components/import", upload.any(), (req, res) => {
  ...
  if (req.files && req.files.length) {
    req.files.forEach((f) =>
      zip.addFile(`uploaded/${f.originalname}`, f.buffer),
    );
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="${importFolder}.zip"`,
  );
  res.send(out);
});
...
router.post("/pmc/:pmc/download", (req, res) => {
  const { pmc } = req.params;
  const types = ["Table", "Session", "Script", "Function", "Domain", "Report"];

  const comps = Array.from({ length: 10 }).map((_, i) => ({
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="${pmc}-export.zip"`,
  );
  res.send(out);
});
...
router.post("/vrcs/:vrc/projects/validate", (req, res) => {
  ...
  if (!projectName) {
    return res.json({ valid: false, errorMessage: "projectName required" });
  }
  res.json({
    valid: true,
...
// Close project
router.post("/vrcs/:vrc/projects/close", (req, res) =>
  res.json({ success: true }),
);

// Script upload
router.post(
  "/vrcs/:vrc/components/Script/:scriptName/source",
  upload.any(),
  (req, res) => {
    const id = generateId();
    res.json({
      script: req.params.scriptName,
      vrc: req.params.vrc,
      path: `/mock/path/${id}`,
      success: true,
    });
...
router.post("/vrcs/:vrc/components/Script/:scriptName/compile", (req, res) => {
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="${req.params.scriptName}-compile.zip"`,
  );
  res.send(out);
});
